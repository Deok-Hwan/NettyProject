...
public class NettyNote {
	 ...
	 * 
	 * 네트워크 어플리케이션은
	 * 네트워크포트 / 소켓 모드와 소켓 옵션 / 소켓의 데이터를 처리하는 스레드 / 프로토콜
	 * 아파치 톰캣은
	 * XML을 사용하여 톰켓 커넥터가 사용할 입출력 모드와 스레드 수, TCP 서비스 포트를 설정
	 * 또한 웹 서버이므로 HTTP 프로토콜을 사용하여 웹 서비스 관리자를 위한 계정을 생성
	 * 프로토콜은 채널 파이프라인에 등록되는 인코더와 디코더에서 처리
	 * 
	 * 
